    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            ...
            else
            {
                ...
                {
                    m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" このプレイリスト"+(playlist.getName()==null ? "" : "(**"+playlist.getName()
                            +"**) ")+"のすべてのエントリが許可されている最大長 (`"+bot.getConfig().getMaxTime()+"`) を超えていたため追加できませんでした。")).queue();
                }
                else
                {
                    m.editMessage(FormatUtil.filter(event.getClient().getSuccess()+" "
                            +(playlist.getName()==null?"プレイリストを検出し": "プレイリスト **"+playlist.getName()+"** を検出し")+"、`
                            + playlist.getTracks().size()+"` 件のエントリをキューに追加しました！"
                            + (count<playlist.getTracks().size() ? "\n"+event.getClient().getWarning()+" 許可されている最大長 (`"
                            + bot.getConfig().getMaxTime()+"`) を超えるトラックは除外されています。" : ""))).queue();
                }
            }
        }
