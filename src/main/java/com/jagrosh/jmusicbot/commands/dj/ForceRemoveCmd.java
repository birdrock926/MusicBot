...
public class ForceRemoveCmd extends DJCommand
{
    public ForceRemoveCmd(Bot bot)
    {
        ...
        this.help = "指定したユーザーの曲をキューからすべて削除します";
        this.arguments = "<user>";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        if(found.isEmpty())
        {
            event.replyError("ユーザーが見つかりませんでした！");
            return;
        }
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** の曲はキューにありません！");
        }
        else
        {
            event.replySuccess("`"+count+"` 件を " + FormatUtil.formatUsername(target) + " から削除しました。");
        }
    }
}
